...
class PageObject {
    ...
    expectVisibleTextWithinMovieCard(movieId: Movie['id'], phrases: (RegExp | string)[]){
        const movieCard = this.getMovieCard(movieId)
        for (const phrase of phrases){
            const element = getByText(movieCard, phrase)
            expect(element).toBeInTheDocument()
        }
    }

    expectMovieImageURL(movieId: Movie['id'], movieURL: string){
        const movieCard = this.getMovieCard(movieId)
        const imgElement = movieCard.querySelector("img") as HTMLImageElement
        expect(imgElement.src).toContain(movieURL)
    }
